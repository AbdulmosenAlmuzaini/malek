...
export default function AddOperation() {
    ...
    const save = async () => {
        if (!form.date) return showToast("يرجى اختيار التاريخ", "error");
        // Allowing 0 amount as per user request to "save anyway"

        try {
            setLoading(true);
            const formData = new FormData();

            // Append all fields, ensure null/undefined are empty strings for FormData if needed
            // but here backend handles nulls if we send them or omit them.
            // Better to only append what we have or send explicit null values.
